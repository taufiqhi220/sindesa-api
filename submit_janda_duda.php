...
if ($res_user && $user = mysqli_fetch_assoc($res_user)) {
    ...
    $penyebab = $_POST['penyebab_status'] ?? '';
    $jenis_kelamin = $_POST['jenis_kelamin'] ?? '';
    $statusTeks = (strtolower($jenis_kelamin) == 'laki-laki') ? 'Duda' : 'Janda';

    // Buat data_tambahan JSON sesuai format website
    $form_data = [
        'nik_pemohon' => $nik,
        'nama_pemohon' => $_POST['nama'] ?? '',
        'tempat_lahir_pemohon' => $_POST['tempat_lahir'] ?? '',
        'tanggal_lahir_pemohon' => $_POST['tanggal_lahir'] ?? '',
        'jenis_kelamin_pemohon' => $jenis_kelamin,
        'penyebab_status' => $penyebab,
        'alamat_pemohon' => $_POST['alamat'] ?? '',
        'nama_mantan' => $_POST['nama_mantan'] ?? '',
        'tahun_berpisah' => $_POST['tahun_berpisah'] ?? '',
        'alamat_mantan' => $_POST['alamat_mantan'] ?? '',
    ];
    ...
    $penyebab_escaped = mysqli_real_escape_string($conn, $penyebab);
    $keperluan = "Surat Keterangan $statusTeks ($penyebab_escaped)";
    ...
    if ($edit_id > 0) {
        ...
        $sql = "INSERT INTO pengajuan_surats (user_id, jenis_surat, keperluan, token_verifikasi, status, data_tambahan, created_at, updated_at)
            VALUES ('$user_id', 'keterangan_janda_duda', '$keperluan', '$token', 'menunggu_verifikasi', '$data_tambahan_escaped', NOW(), NOW())";
    }
    if (mysqli_query($conn, $sql)) {
        api_response(["success" => true, "message" => "Pengajuan Surat Keterangan Janda/Duda berhasil dikirim"]);
    } else {
